...
export default function App() {
    ...
    if (!productsLoading && products.length === 0 && !seeded.current) {
      ...
      (async () => {
        for (const p of SEED_PRODUCTS) {
          try {
            await window.storage.set(`product:${p.id}`, JSON.stringify({ ...p, createdAt: Date.now() }), true);
          } catch {
            /* ignore */
          }
        }
  ...
  const submitOrder = async () => {
    ...
    try {
      const res = await window.storage.set(`order:${order.id}`, JSON.stringify(order), true);
      ...
      for (const item of cartItems) {
        ...
        if (current) {
          ...
          await window.storage.set(`product:${current.id}`, JSON.stringify(updated), true);
        }
      }
      refreshProducts();
      setSubmitted(true);
  ...
  const saveProduct = async (product) => {
    ...
    await window.storage.set(`product:${product.id}`, JSON.stringify(toSave), true);
    refreshProducts();
  };
